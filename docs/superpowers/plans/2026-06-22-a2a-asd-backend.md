# a2a-mcp asd backend Implementation Plan
...
**Goal:** Extend `mcps/a2a` so a `{"kind":"asd"}` config source auto-registers live `asd` sessions as A2A servers (visible in `a2a_list`, dispatchable via `a2a_dispatch`), with the HTTP path unchanged.

**Architecture:** Refactor the flat registry into a list of `Source`s (by `kind`) + a `resolve()` that expands them into `ResolvedServer`s — http sources pass through; a asd source runs `asd ls --json` and emits one server per session. `a2a_list`/`a2a_dispatch` resolve on every call. Asd dispatch drives the session through an injectable `runAsd` seam: peek-before → send → wait → peek-after → scrollback line-delta + trim.

**Tech Stack:** Go 1.23, `modelcontextprotocol/go-sdk` v0.8.0, `net/http`, `os/exec`.

Spec: `docs/superpowers/specs/2026-06-22-a2a-asd-backend-design.md`. Branch: `feat/a2a-asd-backend` (off `main`). Only `mcps/a2a/**` changes.

---

## File Structure
- Modify `mcps/a2a/a2a.go` — replace `Server`/`Registry` with `Source`/`ResolvedServer`; add `loadSources`, `resolve`, the `runAsd` seam + `asdSession`; change `ServerView` (+`Kind`), `runList`, `a2aClient.send` signature, `runDispatch`; add `dispatchAsd` (Task 2).
- Modify `mcps/a2a/a2a_test.go` — update existing tests to the new signatures; add `resolve`/`dispatchAsd` tests.
...
## Task 1: Sources/resolve model + asd auto-registration in `a2a_list`

After this task: `a2a_list` shows http servers AND every live asd session; `a2a_dispatch` works for http and returns a clear "not yet implemented" for asd (filled in Task 2).
...
Replace the existing `a2a_test.go` content's registry/list/dispatch parts with the new model. Key changes: `loadRegistry`→`loadSources` (returns `[]Source`), `runList` takes `[]ResolvedServer`, `runDispatch` takes `[]Source`, and add `resolve` tests with a stubbed `runAsd`. Full new test file:
...
// stubAsd swaps runAsd for tests; returns a restore func.
func stubAsd(fn func(args ...string) ([]byte, int, error)) func() {
	prev := runAsd
	runAsd = func(_ context.Context, args ...string) ([]byte, int, error) { return fn(args...) }
...
func TestResolveAsdFailureKeepsHTTP(t *testing.T) {
	defer stubAsd(func(args ...string) ([]byte, int, error) { return nil, 1, nil })() // asd ls fails
...
func TestRunDispatchAsdNotYetImplemented(t *testing.T) {
	defer stubAsd(func(args ...string) ([]byte, int, error) {
		return []byte(`[{"name":"build","title":"t"}]`), 0, nil
	})()
	if _, err := runDispatch(context.Background(), []Source{{Kind: "asd"}}, newA2AClient(nil), DispatchInput{AgentName: "build", Prompt: "hi"}); err == nil {
		t.Fatal("asd dispatch should error until Task 2")
	}
}
```
...
// runAsd is the single exec seam for the `asd` CLI; tests stub it.
...
// resolve expands sources into live servers. http passes through; a asd source
// runs `asd ls --json` and emits one server per session. asd failures are logged
...
func resolve(ctx context.Context, sources []Source) []ResolvedServer {
	...
	for _, s := range sources {
		...
		if err != nil || code != 0 {
			log.Printf("a2a: asd ls failed (skipping asd sessions): code=%d err=%v", code, err)
			continue
		}
		var sessions []asdSession
		if len(bytes.TrimSpace(stdout)) > 0 {
			if err := json.Unmarshal(stdout, &sessions); err != nil {
				log.Printf("a2a: parse asd ls --json: %v", err)
				continue
			}
		}
		wt := s.WaitTimeout
...
```
Replace `runDispatch` to resolve + branch (asd is a placeholder error this task):
```go
func runDispatch(ctx context.Context, sources []Source, c *a2aClient, in DispatchInput) (DispatchOutput, error) {
	...
	switch target.Kind {
	case kindAsd:
		return DispatchOutput{}, fmt.Errorf("asd dispatch not yet implemented")
	default:
...
cd /root/workspace/master/myclaw && go build ./mcps/... 2>/dev/null; go build ./mcps/echo/... ./mcps/ping/... ./mcps/asd/... ./mcps/a2a/... && go test ./mcps/echo/... ./mcps/ping/... ./mcps/asd/... ./mcps/a2a/...
```
All a2a tests pass (incl. resolve + http dispatch); echo/ping/asd unaffected.
...
git commit -m "feat(mcps/a2a): source/resolve model + auto-register asd sessions in a2a_list"
```
...
## Task 2: `dispatchAsd` — drive a session + scrollback delta

**Files:** Modify `mcps/a2a/a2a.go`, `mcps/a2a/a2a_test.go`.

- [ ] **Step 1: Add failing dispatchAsd tests**
...
func TestDispatchAsdSessionMissing(t *testing.T) {
	defer stubAsd(func(args ...string) ([]byte, int, error) { return nil, 3, nil })() // exit 3
	if _, err := dispatchAsd(context.Background(), "ghost", "hi", "5s"); err == nil {
...
```
Also flip `TestRunDispatchAsdNotYetImplemented` (Task 1) into a success case, or replace it: with `dispatchAsd` wired in, dispatching to a asd-kind server should now route through it. Add:
```go
...
```
(Remove the old `TestRunDispatchAsdNotYetImplemented`.)

- [ ] **Step 2: Run — expect FAIL** (`dispatchAsd` undefined).

- [ ] **Step 3: Implement `dispatchAsd` + wire the asd branch**

Add to `a2a.go` (imports `strings` already present):
```go
// dispatchAsd types the prompt into a asd session, waits for it to settle, and
...
func dispatchAsd(ctx context.Context, session, prompt, waitTimeout string) (string, error) {
	...
	if _, code, err := runAsd(ctx, "send", session, "--text", prompt, "--enter"); err != nil {
		return "", fmt.Errorf("asd not available: %w", err)
	} else if e := asdDispatchErr(session, code); e != nil {
		return "", e
	}
	// wait is a settle hint; a timeout (exit 4) is non-fatal.
	if _, _, err := runAsd(ctx, "wait", session, "--idle", "--timeout", waitTimeout); err != nil {
		return "", fmt.Errorf("asd not available: %w", err)
	}
...
func asdPeek(ctx context.Context, session string) (string, error) {
	out, code, err := runAsd(ctx, "peek", session, "--scrollback")
	if err != nil {
		return "", fmt.Errorf("asd not available: %w", err)
	}
	if e := asdDispatchErr(session, code); e != nil {
		return "", e
	}
	return string(out), nil
}

func asdDispatchErr(session string, code int) error {
	switch code {
...
Then wire the asd branch in `runDispatch`:
```go
	case kindAsd:
...
- [ ] **Step 4: Run — expect PASS**: `cd mcps/a2a && go test ./... -v && go vet ./... && go build ./...` (all tests incl. the 3 dispatchAsd + the asd-route test). Then the no-blast-radius check (echo/ping/asd).

- [ ] **Step 5: Commit**

```bash
git add mcps/a2a/a2a.go mcps/a2a/a2a_test.go
git commit -m "feat(mcps/a2a): asd session dispatch via scrollback delta"
```
(Trailer as above.)

---

## Task 3: Real-asd smoke

**Files:** none (verification; fix-forward into `mcps/a2a` if the live delta heuristic misbehaves).

- [ ] **Step 1: Build + drive a real asd session through the a2a binary**

```bash
cd /root/workspace/master/myclaw/mcps/a2a && go build -o /tmp/a2a-mcp .
asd new a2asmoke -d -- bash
D=$(mktemp -d); printf '[{"kind":"asd"}]' > "$D/c.json"
...
asd peek a2asmoke --scrollback | wc -l
asd send a2asmoke --text 'echo SMOKE_OK' --enter
asd wait a2asmoke --idle --timeout 5s
asd peek a2asmoke --scrollback     # confirm "SMOKE_OK" appears in the new lines
asd kill a2asmoke
```
Expected: `a2a_list` includes the live session; `a2a_dispatch` to it returns text containing `SMOKE_OK`, with the prompt-echo and trailing shell prompt trimmed. If the real delta/trim is off (e.g. `asd peek --scrollback` line shape differs from the test fixtures), fix `dispatchAsd`/`trimDelta` in `a2a.go`, re-run `go test ./mcps/a2a/...`, and report. Clean up the `a2asmoke` session.

- [ ] **Step 2: Commit any fix-forward** (only if Step 1 required a change)

```bash
git add mcps/a2a/a2a.go mcps/a2a/a2a_test.go
git commit -m "fix(mcps/a2a): align asd scrollback delta with real asd output"
```
...
**Spec coverage:** sources-by-kind model + `kind` default http + backward compat (Task 1 `Source`/`kind()`/`loadSources` + `TestLoadSources`) ✓; `resolve` http-passthrough + asd `asd ls` expansion + asd-failure-keeps-http (Task 1 + 3 resolve tests) ✓; `a2a_list` auto-registers live sessions (Task 1 main + runList incl Kind) ✓; `a2a_dispatch` resolve+branch, http unchanged (Task 1) ✓; `dispatchAsd` peek-before/send/wait/peek-after line-delta + trim, timeout non-fatal, session-missing error (Task 2 + 3 tests) ✓; `runAsd` seam + stub testing (Task 1) ✓; token never leaked (`ServerView` no token, `TestRunListOmitsTokenIncludesKind`) ✓; real-asd smoke (Task 3) ✓. Out-of-scope (filtering, per-call timeout, marker, streaming) absent.

**Placeholder scan:** Task 2's `trimDelta` block is intentionally followed by an IMPLEMENTER note pinning the exact 3-rule behavior + "the tests are the contract" — the over-fiddly draft must be simplified, not shipped as-is. Task 3's handshake step gives a concrete fallback (drive the asd pieces directly) when no MCP-handshake helper exists. No "TBD"/silent-cap placeholders.

**Type consistency:** `Source{Kind,Name,Description,Endpoint,AuthToken,WaitTimeout}` + `Source.kind()`; `ResolvedServer{Name,Description,Kind,Endpoint,AuthToken,Session,WaitTimeout}`; `resolve(ctx,[]Source)[]ResolvedServer`; `runList([]ResolvedServer)`; `runDispatch(ctx,[]Source,*a2aClient,DispatchInput)`; `(*a2aClient).send(ctx,endpoint,authToken,prompt string)`; `dispatchAsd(ctx,session,prompt,waitTimeout string)`; `runAsd` seam `func(ctx,...string)([]byte,int,error)` — all consistent across a2a.go, the tests, and main.go. The Task-1 placeholder asd error is removed in Task 2.

...
**Goal:** A standalone `mcps/asd` MCP server exposing the `asd` terminal-multiplexer automation surface (`asd_ls/new/send/peek/wait/kill`) as typed MCP tools.

**Architecture:** Mirrors `mcps/echo` and `mcps/ping`: one Go module, `modelcontextprotocol/go-sdk` stdio server. All tools shell out to the `asd` binary through a single injectable `runAsd` seam. Each tool = a pure `argsFor*(in) ([]string, error)` (validation + argv, unit-tested) + a `run*(ctx, in)` handler (calls `runAsd`, maps exit codes, parses JSON). Tests stub `runAsd` — no real binary or sessions needed.

**Tech Stack:** Go 1.23, `github.com/modelcontextprotocol/go-sdk` v0.8.0, `os/exec`.

Spec: `docs/superpowers/specs/2026-06-22-asd-mcp-design.md`. asd exit codes: 0 ok · 1 error · 2 usage · 3 no-such-session · 4 wait-timeout.

---

## File Structure
- `mcps/asd/go.mod`, `mcps/asd/go.sum` — module `github.com/benenen/myclaw/mcps/asd`.
- `mcps/asd/asd.go` — types, `runAsd` seam, `argsFor*` builders, `run*` handlers, exit-code mapping.
- `mcps/asd/asd_test.go` — argv-builder + validation + exit-mapping + JSON-parse tests (stubbed `runAsd`).
- `mcps/asd/main.go` — server + 6 `mcp.AddTool` registrations + stdio transport.
- `go.work` — add `./mcps/asd` to the `use (...)` block.

⚠️ **Shared branch:** this is on `feat/per-bot-mcp-servers` next to a concurrent session's `mcps/echo` + `mcps/ping`. ONLY create new `mcps/asd/**` files and add ONE line to `go.work`. NEVER `git add -A`; stage explicit paths. Do not touch `mcps/echo`, `mcps/ping`, `Makefile`, or the per-bot-MCP files.
...
**Files:**
- Create: `mcps/asd/go.mod`, `mcps/asd/asd.go`, `mcps/asd/asd_test.go`
- Modify: `go.work`

- [ ] **Step 1: Create the module**

```bash
cd mcps/asd 2>/dev/null || mkdir -p mcps/asd
cat > mcps/asd/go.mod <<'EOF'
...
EOF
```
Add `./mcps/asd` to the `use (...)` block in the repo-root `go.work` (keep the existing `.`, `./mcps/echo`, `./mcps/ping` lines).

- [ ] **Step 2: Write the failing test `mcps/asd/asd_test.go`**
...
- [ ] **Step 3: Run — expect FAIL** (`go test ./mcps/asd` → undefined `argsForLs` etc.)

- [ ] **Step 4: Write `mcps/asd/asd.go` (types + builders only for now)**
...
// runAsd is the single exec seam; handlers (Task 2) call it, tests stub it.
var runAsd = func(ctx context.Context, args ...string) (stdout []byte, stderr []byte, exitCode int, err error) {
	return nil, nil, 0, fmt.Errorf("runAsd not implemented") // replaced in Task 2
}
```
...
GOWORK=off go test ./mcps/asd/ -v    # or: (cd mcps/asd && go test ./... -v)
```
Expected: all `TestArgsFor*` PASS. Also `(cd mcps/asd && go mod tidy && go build ./...)` to populate `go.sum` (deps are already in the module cache from echo/ping).

- [ ] **Step 6: Commit**

```bash
git add mcps/asd/go.mod mcps/asd/go.sum mcps/asd/asd.go mcps/asd/asd_test.go go.work
git commit -m "feat(mcps/asd): scaffold module + asd argv builders + validation"
```
...
**Files:**
- Modify: `mcps/asd/asd.go` (replace the `runAsd` stub with the real exec; add `run*` handlers + `asdError`)
- Modify: `mcps/asd/asd_test.go` (handler tests with stubbed `runAsd`)
- Create: `mcps/asd/main.go`

- [ ] **Step 1: Add failing handler tests to `asd_test.go`**
...
func stub(out string, code int) func() {
	prev := runAsd
	runAsd = func(_ context.Context, _ ...string) ([]byte, []byte, int, error) {
...
func TestSendValidationErrorSkipsExec(t *testing.T) {
	called := false
	prev := runAsd
	runAsd = func(_ context.Context, _ ...string) ([]byte, []byte, int, error) { called = true; return nil, nil, 0, nil }
	defer func() { runAsd = prev }()
...
- [ ] **Step 2: Run — expect FAIL** (`run*` undefined): `GOWORK=off go test ./mcps/asd/`

- [ ] **Step 3: Replace the `runAsd` stub + add handlers in `asd.go`**

Replace the placeholder `runAsd` var with the real one, and add the handlers + `asdError`:
```go
...
```
Update the test stub signature note: the real `runAsd` returns `(stdout, stderr, exitCode, err)` — the Task-2 test `stub()` already matches this 4-value shape.

- [ ] **Step 4: Create `mcps/asd/main.go`**
...
```bash
cd /root/workspace/master/myclaw/mcps/asd
...
```bash
cd /root/workspace/master/myclaw
git add mcps/asd/asd.go mcps/asd/asd_test.go mcps/asd/main.go mcps/asd/go.sum
git commit -m "feat(mcps/asd): tool handlers, exit-code mapping, and stdio server"
```
(Trailer as above.)

---

## Task 3: Real-asd smoke verification

**Files:** none (verification only; fix-forward into `mcps/asd` if a real run reveals a flag/parse mismatch).

- [ ] **Step 1: Build the server binary**

```bash
cd /root/workspace/master/myclaw/mcps/asd && go build -o /tmp/asd-mcp .
```

- [ ] **Step 2: Drive it against the real `asd` via the canonical loop** (proves argv + parsing match the installed asd)

Run the asd CLI directly to confirm the argv this server builds is accepted by the installed asd, and that `--json` shapes parse:
```bash
asd new asdmcptest -d -- bash
asd send asdmcptest --text 'echo hi' --enter
asd wait asdmcptest --idle --timeout 5s
asd peek asdmcptest --json | head
asd ls --json
asd kill asdmcptest
```
Expected: a session name printed; `peek --json` keys = `session,title,rows,cols,cursor,screen`; `ls --json` items = `name,attached,idle_ms,title`. If any flag/shape differs from the spec, fix `asd.go`'s builder/struct and re-run `go test ./...`.

- [ ] **Step 3 (optional): JSON-RPC smoke of the server itself**

Pipe an MCP `tools/list` request to `/tmp/asd-mcp` on stdin and confirm the six `asd_*` tools are listed on stdout (diagnostics on stderr). If tooling for a raw MCP handshake isn't readily available, rely on Step 2 + the unit tests and note it.

- [ ] **Step 4: Commit any fix-forward** (only if Step 2 required a change)

```bash
git add mcps/asd/asd.go mcps/asd/asd_test.go
git commit -m "fix(mcps/asd): align argv/parse with installed asd"
```
...
**Spec coverage:** all six tools (Tasks 1–2) ✓; injectable `runAsd` seam ✓; pure `argsFor*` + handlers split ✓; exit-code mapping 0/3/4/other ✓ (Task 2 + tests); JSON parse for ls/peek ✓; validation (send text⊕keys, wait mode/text, kill name⊕all) ✓ (Task 1 tests); `go.work` entry ✓; stdio + stderr-logging convention ✓ (main.go); real-asd smoke ✓ (Task 3). Out-of-scope (attach/ui/rename/restore/version) correctly absent.

**Placeholder scan:** the Task-1 `runAsd` is an intentional, clearly-labelled stub replaced in Task 2 (not a plan placeholder — its replacement is fully specified). No TBD/TODO/"handle errors" hand-waves.

**Type consistency:** `runAsd` signature `(stdout, stderr []byte, exitCode int, err error)` is consistent between the Task-2 real impl, the Task-2 `stub()`, and all `run*` handlers. Struct/field names (`LsOutput.Sessions`, `Session.IdleMs`, `PeekOutput.Cursor.Col`, `WaitOutput.Matched`, `NewOutput.Name`) match across builders, handlers, tests, and `main.go` tool registrations. `argsFor*` all return `([]string, error)`.

**Note:** Task-1 Step 5 runs the arg tests before handlers exist — those tests only reference `argsFor*` + types, so they compile and pass with the Task-1 `asd.go` (the `runAsd` stub is unused by them). The handler tests are added in Task 2.
